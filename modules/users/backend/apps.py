...
class UsersConfig(AppConfig):
    ...
    def _add_sdk_to_path(self):
        ...
        try:
            # Get project root (from modules/users/backend -> go up 3 levels)
    ...
    def _initialize_module(self):
        ...
        try:
            ...
            # Initialize module
            self.unibos_module = UnibosModule('users')

            # Log initialization
            import logging
            logger = logging.getLogger(__name__)
            logger.info(f"✓ Initialized UNIBOS module: users v{self.unibos_module.manifest.get('version')}")
